...
export async function runCycle(candles, currentState={}) {
  ...
  for (const pos of openPositions) {
    ...
    if (exitCheck.shouldPartial && s.partialTpEnabled !== false) {
      exitDecisions.push({
        position: pos, reason: 'partial_tp1',
        isPartial: true, partialPct: exitCheck.partialPct,
        pnl: exitCheck.pnl,
      });
      addLog(
        `💰 PARTIAL TP1 — Jual 50% ${pair.replace('_idr','').toUpperCase()} @ ${close.toLocaleString('id-ID')} | ` +
        `+Rp ${Math.abs(exitCheck.pnl/1000).toFixed(1)}K | Sisa 50% trailing`,
        'profit'
      );
      continue;
    }
    ...
    if (exitCheck.shouldBreakeven && s.breakevenEnabled !== false) {
      exitDecisions.push({
        position: pos, reason: 'breakeven_set',
        isBreakeven: true, newStopLoss: exitCheck.newStopLoss,
        pnl: 0,
      });
      addLog(
        `🔒 BREAKEVEN — SL digeser ke entry ${pair.replace('_idr','').toUpperCase()} | ` +
        `Posisi aman, tidak bisa rugi`,
        'system'
      );
      continue;
    }

    // ── Normal exit: SL / TP / Trailing / Time-based ─────────────────────
    if (exitCheck.shouldClose) {
      ...
      const tag   = exitCheck.reason === 'time_exit'     ? '⏰ TIME EXIT' :
                    exitCheck.reason === 'max_hold_time'  ? '⏰ MAX HOLD' :
                    exitCheck.reason.toUpperCase().replace(/_/g,' ');
      addLog(
        `${emoji} ${tag} | ${exitCheck.pnl>=0?'+':''}Rp ${Math.abs(exitCheck.pnl/1000).toFixed(1)}K`,
    ...
    if (s.smartExitEnabled !== false && signal) {
      const reversal = checkSignalReversal(pos, close, signal);
      if (reversal.shouldExit) {
        exitDecisions.push({ position: pos, reason: reversal.reason, pnl: reversal.pnl });
        const emoji = reversal.pnl >= 0 ? '✅' : '❌';
        const tag   = reversal.reason === 'smart_signal_exit' ? '🧠 SMART EXIT' : '✂️ EARLY CUT';
        addLog(
          `${emoji} ${tag} — Sinyal berbalik, keluar lebih awal | ` +
          `${reversal.pnl>=0?'+':''}Rp ${Math.abs(reversal.pnl/1000).toFixed(1)}K`,
          reversal.pnl >= 0 ? 'profit' : 'loss'
        );
      }
    }
  }
  ...
  if (allowed && signal.action === 'BUY' && openPositions.length === 0) {
    if (Date.now() - botState.lastActionTime < cooldown) {
      ...
      // ── PROFIT COMPOUNDING: calculatePositionSize sudah handle streak di dalam ──
      // BUG FIX v5: Kirim signalGrade agar grade A+/A/B/C berpengaruh ke size
      // BUG FIX v5: streakBonus tidak lagi digandakan (sebelumnya ada di sini DAN di calculatePositionSize)
      const signalGrade   = signal.context?.momentum?.grade || 'C';
      const sizing = calculatePositionSize(
        balance, openPositions.length,
        { consecutiveLosses: botState.consecutiveLosses, totalPnl: botState.totalPnl, consecutiveWins: botState.consecutiveWins },
        signalGrade,
      );

      const equityRiskMult = equityMode.riskMult;
      // Equity mode mult tetap berlaku, streak sudah dihitung di dalam sizing
      const finalMult      = Math.min(equityRiskMult, 1.15); // Max 115% equity mode
      const adjustedAmount = Math.floor(sizing.idrAmount * finalMult);
      const winStreak      = botState.consecutiveWins; // untuk logging
      ...
      if (adjustedAmount < (s.minTradeIDR || 1000)) {
        ...
        // BUG FIX v5: R:R minimum berbeda per mode
        // Ultra Profit: lebih longgar (R:R 1.0 cukup — kejar frekuensi)
        // Ultra Light:  lebih ketat  (R:R 2.0 minimum — hanya trade bagus)
        // Max Profit:   sedang       (R:R 1.5 minimum — balance antara frekuensi & kualitas)
        // Normal:       R:R 1.2
        const activeMode = getActiveProfitMode();
        const minRR = activeMode === 'ultra_profit' ? 1.0
                    : activeMode === 'ultra_light'  ? 2.0
                    : activeMode === 'max_profit'   ? 1.5
                    : 1.2;

        if (rr < minRR) {
          addLog(`⚡ Skip entry — R:R ${rr.toFixed(2)}x < min ${minRR}x (${activeMode})`, 'warning');
        } else {
          entryDecision = {
          ...
          addLog(
            ...
            `${contextStr || 'standard'} | ${equityMode.mode} | ${activeMode} | ${session.sessionName}`,
            'buy'
          );
